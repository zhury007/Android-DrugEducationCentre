...
import com.zhuruilin.drugeducationcentre.http.HttpUtils;
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            try {
                ...
                params_post.put("username", mEmail);
                params_post.put("password2", mPassword);

                //服务器请求路径
                String strUrlPath = "http://" + getResources().getString(R.string.ServerAddr) + "/reg";
                String strResult = HttpUtils.submitPostData(strUrlPath, params_post, "utf-8");
                Log.d("http_post",strUrlPath + strResult);


                JSONObject jsStr = new JSONObject(strResult);
                String jsUid = jsStr.getString("uid");
                String jsTipMsg = jsStr.getString("tipMsg");
                Log.d("login+++++++++", jsStr.toString());
                String jsStatus = jsStr.getString("status");
                if (Integer.parseInt(jsStatus) != 0) {
                    return false;
                }
                UserInfo.getInstance().doLogin("null", jsUid);
